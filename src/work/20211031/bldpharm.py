...
def getProdInfo(sope, pInfo):
	meta = sope.find("meta", attrs={"name":"keywords"})
	if meta!=None:
		infos = meta["content"].split("|")
		for infoStr in infos:
			if infoStr.find("Purity") > -1:
				pInfo["Purity/Specification"] = infoStr.replace("Purity(", "").replace(")", "")
	pInfo["Storage"] =  getNodeText(sope.find("td", attrs={"id":"storage"}))
...
def getProductList(url, keywork, products):
	...
	getProdIndoFromUrl(url, pInfo)
	products.append(pInfo.copy())

excelFileName="bldpharm.xlsx"
wb = Workbook()
workSheet = wb.active
products = []

headers=[
	'keywork','Purity/Specification','Storage'
]
...
with open(fileName,'r') as file_to_read:
	...
	while True:
		...
		if not lines:
    			break
		print(lines)
		getProductList("https://www.bldpharm.com/products/"+lines.replace("\r", "").replace("\n", "").strip()+".html", lines, products)
