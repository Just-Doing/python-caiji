...
def getProdInfo(sope, pInfo):
	...
	for th in ths:
		...
		if title =="LogP":
			pInfo["LogP"] =  getNodeText(th.nextSibling.nextSibling)
		if title =="MDL number":
			pInfo["MDL number"] =  getNodeText(th.nextSibling.nextSibling)
		if title =="Density":
			pInfo["Density"] =  getNodeText(th.nextSibling.nextSibling)
		if title =="Boiling Point":
			pInfo["Boiling Point"] =  getNodeText(th.nextSibling.nextSibling)
...
def getProductList(url, keywork, products):
	...
	htmlStr = getHtmlFromUrl(url)
	sope = BeautifulSoup(htmlStr, "html.parser",from_encoding="utf-8")
	link = sope.find("a", attrs={"class", "commpund_name"})
	pInfo = {"keywork": keywork}
	if link !=None:
		getProdIndoFromUrl("https://www.molbase.com"+link["href"], pInfo)
	products.append(pInfo.copy())

excelFileName="molbase.xlsx"
wb = Workbook()
workSheet = wb.active
products = []

headers=[
	'keywork','LogP','MDL number','Density','Boiling Point'
]
...
with open(fileName,'r') as file_to_read:
	...
	while True:
		...
		if not lines:
    			break
		print(lines)
		getProductList("https://www.molbase.com/cas/"+lines.replace("\r", "").replace("\n", "").strip()+".html", lines, products)
