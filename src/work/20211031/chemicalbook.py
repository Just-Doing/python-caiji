...
def getProdInfo(sope, pInfo):
	...
	for li in lis:
		...
		if title.find("storage temp.") > -1 and "storage temp" not in pInfo:
			pInfo["storage temp"] = title.replace("storage temp.", "").replace("\xa0", "")
		if title.find("form") > -1 and "form" not in pInfo:
			pInfo["form"] = title.replace("form", "").replace("\xa0", "")
		if title.find("Chemical Properties") > -1 and "Chemical Properties" not in pInfo:
			pInfo["Chemical Properties"] = title.replace("Chemical Properties", "").replace("\xa0", "")
...
def getProductList(url, keywork, products):
	...
	htmlStr = getHtmlFromUrl(url)
	sope = BeautifulSoup(htmlStr, "html.parser",from_encoding="utf-8")
	links = sope.find_all("li")
	pInfo = {"keywork": keywork}
	for link in links:
		if getNodeText(link).find("CBNumber") > -1:
			pLink = link.find("a")
			getProdIndoFromUrl("https://m.chemicalbook.com"+pLink["href"], pInfo)
			
	products.append(pInfo.copy())

excelFileName="chemicalbook.xlsx"
wb = Workbook()
workSheet = wb.active
products = []

headers=[
	'keywork','storage temp','form','Chemical Properties'
]
...
with open(fileName,'r') as file_to_read:
	...
	while True:
		...
		if not lines:
				break
		print(lines)
		getProductList("https://m.chemicalbook.com/Search_EN.aspx?keyword="+lines.replace("\r", "").replace("\n", "").strip()+"&start=0", lines, products)
# getProductList("https://m.chemicalbook.com/Search_EN.aspx?keyword=25574-11-2&start=0", '', products)
# getProdIndoFromUrl("https://m.chemicalbook.com/ProductChemicalPropertiesCB9838257_EN.htm", {})
