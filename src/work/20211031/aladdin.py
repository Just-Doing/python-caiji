...
def getProdInfo(sope, pInfo):
	applicationth = sope.find("th", attrs={"class":"col label application"})
	solubility = sope.find("th", attrs={"class":"col label solubility"})
	if applicationth != None:
		pInfo['applicationth'] = getNodeText(applicationth.nextSibling.nextSibling)
	if solubility != None:
		pInfo['solubility'] = getNodeText(solubility.nextSibling.nextSibling)
...
def getProductList(url, keywork, products):
	...
	pLink = sope.find("a", attrs={"class":"product"})
	if pLink != None:
		getProdIndoFromUrl(pLink["href"], pInfo)
...
headers=[
	'keywork','applicationth','solubility'
]
...
with open(fileName,'r') as file_to_read:
	...
	while True:
		...
		if not lines:
    			break
		print(lines)
		getProductList("https://www.aladdin-e.com/zh_en/catalogsearch/result/?q="+lines, lines, products)
