...
def getProductInfo(url):
	...
	for spec in specs:
		...
		if len(divs) == 2:
			...
			if len(title) >0:
				title = title.replace(getNodeText(strongName), "")
				pInfo[title] = value
...
getProductList("https://www.mybiosource.com/search/Caenorhabditis+Elegans?type=ELISA+Kit&size=200")
...
headers=['link', 'nav', 'Product Name', 'Catalog #','Full Product Name','Product Gene Name','Species Reactivity','Samples','Preparation and Storage',
	 	'Sample Preparation','Product Note','Other Notes','NCBI GeneID','NCBI Official Symbol','Protein Family'
	 ]


excelUtils.generateExcelMultipleSheet('mybiosource-Kit.xlsx', [
	{
		"name":"mybiosource",
