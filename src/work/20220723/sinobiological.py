...
def getProductInfo(url, products):
	...
	for descSec in descSecs:
		...
		if title != None:
			...
			if titleStr == "Materials provided":
				strongs = value.find_all("strong")
				for strong in strongs:
					reg=re.compile(r'\(Catalog:[\w\W]*\)')
					strongStr = getNodeText(strong)
					if "Capture Antibody" in strongStr:
						matchs = reg.findall(strong.nextSibling)
						if len(matchs) > 0:
							pInfo["Cap Cat#"] = matchs[0].replace("(Catalog:","").replace(")","")
					
					if "Detection Antibody" in strongStr:
						matchs = reg.findall(strong.nextSibling)
						if len(matchs) > 0:
							pInfo["Det Cat#"] = matchs[0].replace("(Catalog:","").replace(")","")
		else:
			if value!= None:
				valueStr = getNodeText(value)
				pInfo["introduction"] = valueStr

	products.append(pInfo.copy());

def getProductList(url, pIndex, products):
	file = open("data"+str(pIndex)+".txt")
	html_code = json.loads(file.read())
	result = html_code["Result"]
	print(len(result))
	for tr in result:
		getProductInfo("https://www.sinobiological.com"+tr["ProductUrl"], products)
...
def theardFun(startPage, endPage, excelFname):
	products = []
	# getProductInfo("https://www.sinobiological.com/pair-set/hemagglutinin-ha-sek001", products)
	getProductList('https://www.sinobiological.com/SearchResultDetails', endPage, products)
	# getProductList('https://www.sinobiological.com/SearchResultDetails', pIndex, products)
	...
	headers=[
		'link','productName','cat','size0','price0','size1','price1','size2','price2','size3','price3','fig','figurl'
		,'Assay type','Specificity','Assay range','Materials provided','Cap Cat#','Det Cat#','Product overview','Shipping'
		,'Storage','introduction','References','title0','PubMedID0','Author0','Year0','Journal0','Application0'
		,'title1','PubMedID1','Author1','Year1','Journal1','Application1'
		,'title2','PubMedID2','Author2','Year2','Journal2','Application2'
		,'title3','PubMedID3','Author3','Year3','Journal3','Application3'
	]
...
try:
	# # _thread.start_new_thread( theardFun, (1, 1, "sinobiological1.xlsx" ) )
	_thread.start_new_thread( theardFun, (1, 2, "sinobiological2.xlsx" ) )
	_thread.start_new_thread( theardFun, (1, 3, "sinobiological3.xlsx" ) )
	_thread.start_new_thread( theardFun, (1, 4, "sinobiological4.xlsx" ) )
	_thread.start_new_thread( theardFun, (1, 5, "sinobiological5.xlsx" ) )
except:
	print ("Error: 无法启动线程")
while 1:
   pass