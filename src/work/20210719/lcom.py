...
def getProductList(url, typeStr, products, isFirst):
	pData = requestJson(url)
	totalPage = math.ceil(pData["iTotalRecords"]/100)
	
	for p in pData["aaData"]:
		getProductInfo("https://www.l-com.com"+p["Url"], typeStr, products)
	if isFirst:
		for pageIndex in range(2, totalPage+1):
			getProductList(url+"&page="+str(pageIndex),typeStr, products, False)


def getTypeList(products):
	types=['Active Optical Cables',
			'Fiber Optic Active Converters/Extenders - Audio/Video',
			...
			'Fiber Optic Passive Splitters/Combiners',
			'Fiber Optic Passive Taps',
			'Fiber Optic Transceivers',
			'Fiber Test and Measurement',
			'Interconnect Miscellaneous',
			'Manual Switches',
			'RF Kits Miscellaneous',
			'Rack Accessories',
			'Rack Panels',
			'Sub-Panels',
			'Test Equipment',
	...
	for typeStr in types:
		pList = 'https://www.l-com.com/nsearchwrapper?Level0=Fiber+Optic&view_type=grid&Category='+typeStr+'&initial_sort=Sortsku:ASC&res_per_page=100'
		getProductList(pList, typeStr, products, True)
...
# getProductInfo("https://www.l-com.com/fiber-optic-adapter-fc-connector-for-900Ã¦m-buffered-fiber-foa-fcb9", 'tes', products)
getTypeList(products)
# getProductList('https://www.l-com.com/nsearchwrapper?Level0=Fiber+Optic&view_type=grid&Category=Active Optical Cables&initial_sort=Sortsku:ASC&res_per_page=100', products, True)
