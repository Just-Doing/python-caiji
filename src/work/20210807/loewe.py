...
def getProductList(url, typeStr, products):
	...
	if len(html_code)>0:
		...
		typeLis = sope.find_all("li", attrs={"class":"accordion"})
		for typeLi in typeLis:
			type2=getNodeText(typeLi.find("a"))
			pList = typeLi.find_all("li", attrs={"class":"submenu"})
			for p in pList:
				pInfo = {}
				pInfo["type1"]=typeStr
				pInfo["type2"]=type2
				pName = getNodeText(p.find("a", attrs={"class":"submenu"}))
				pInfo["product name"]=pName
				baseInfos = p.find_all("p")
				for baseInfo in baseInfos:
					title = getNodeText(baseInfo)
					if title.startswith("Assay Type:"):
						pInfo["Assay Type"] = title.replace("Assay Type:","")
					if title.startswith("Host range:"):
						pInfo["Host range"] = title.replace("Host range:","")
				tableInfoTr = p.find_all("tr")
				Description = ""
				Format = ""
				CatNo = ""
				for tableInfo in tableInfoTr:
					tds = tableInfo.find_all("td")
					if len(tds) == 4:
						Description+= getNodeText(tds[0])+","
						Format+= getNodeText(tds[1])+"/"
						CatNo+= getNodeText(tds[2])+"/"
				pInfo["Description/Format/Cat.No"]="("+Description+");("+Format+");("+CatNo+")"
				products.append(pInfo.copy())
			

excelFileName="loewe.xlsx"
wb = Workbook()
workSheet = wb.active
products = []

headers=[
	'type1','type2','product name','Assay Type','Host range','Description/Format/Cat.No'
]


getProductList("https://www.loewe-info.com/categories/elisa-sets-kits-and-controls/plant-viruses.html", 'Plant Viruses', products)
getProductList("https://www.loewe-info.com/categories/elisa-sets-kits-and-controls/plant-bacteria.html", 'Plant Bacteria', products)
getProductList("https://www.loewe-info.com/categories/elisa-sets-kits-and-controls/plant-fungi.html", 'Plant Fungi', products)
