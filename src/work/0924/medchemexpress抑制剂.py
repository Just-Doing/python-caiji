...
def getProductInfo(url, pInfo, products):
    ...
    for detailTr in detailTrs:
        title = getNodeText(detailTr.find("th")).strip()
        val = getNodeText(detailTr.find("td"))
        pInfo[title] = val
    pInfo["link"] = url
...
headers = [
    'link', 'nav', 'cat', 'Product Name', 'target', 'research', 'description', 'size', '生物活性', 'Formula', 'CAS 号', '运输条件', '储存方式', '纯度 & 产品资料', '参考文献', '体外研究(In Vitro)', '体内研究(In Vivo)', '分子量', '结构分类',
    '来源'
]

excelFname = "medchemexpress抑制剂.xlsx"
products = []

getProductList("https://www.medchemexpress.cn/search.html?q=diabetes&ft=&fa=&fp=&fsp=&ftag=&fsc=&type=inhibitors-and-agonists?enCountryName=United%20States", products)
