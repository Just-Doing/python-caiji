...
def getHtmlFromUrl(url, type="get", para={}):
    ...
    try:
        url = urllib.parse.quote(
            url, safe=string.printable).replace(' ', '%20')
...
def getProdIndoFromUrl(url, products):
...
def getProductList(url, products):
    ...
    pLinks = sope.find_all("li", attrs={"class": "item product product-item"})
    for pLinkArea in pLinks:
        pLink = pLinkArea.find("a")
        getProdIndoFromUrl(pLink["href"], products)


excelFileName = "aladdin.xlsx"
wb = Workbook()
workSheet = wb.active
products = []

headers = [
    'link', '类型', '英文名称', '英文别名', '规格或纯度', '闪点（摄氏）', '折光率', 'CAS编号', '熔点', '沸点', '密度', '分子量', '分子式', 'Smiles'
]

for pIndex in range(1, 64):
    getProductList("https://www.aladdin-e.com/zh_cn/biochemicals-and-reagents/lipids.html?p=" +
                   str(pIndex)+"&product_list_limit=45", products)

for pIndex in range(1, 49):
    getProductList("https://www.aladdin-e.com/zh_cn/materials-science/bioscience/carbohydrates.html?p=" +
                   str(pIndex)+"&product_list_limit=45", products)

for pIndex in range(1, 6):
    getProductList("https://www.aladdin-e.com/zh_cn/chemicals-and-biochemicals/bioscience/vitamins.html?p=" +
                   str(pIndex)+"&product_list_limit=45", products)
# getProductList("https://www.aladdin-e.com/zh_cn/biochemicals-and-reagents/lipids.html?p=1&product_list_limit=45&product_list_mode=list", products)
# getProdIndoFromUrl("https://www.aladdin-e.com/zh_cn/s100121.html", products)
