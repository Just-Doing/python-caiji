...
def getProductInfo(url):
	...
	for tr in trs:
		th = tr.find("th")
		td = tr.find("td")
		pInfo[getNodeText(th)] = getNodeText(td)

	products.append(pInfo.copy());
	print(pInfo)

def getProductList(url):
	html_code = requestJson(url)
	proList = html_code.find_all("tr")
	for tr in proList:
		pLink = tr.find("a")
		getProductInfo("https://web.expasy.org" + pLink["href"])
...
headers=[
	'link','Cell line name','Synonyms','Accession','Comments','Species of origin','Hierarchy','Category','Sex of cell','Age at sampling',
	'Cell line collections'
]

getProductList("https://web.expasy.org/cgi-bin/cellosaurus/search")
# getProductInfo("https://web.expasy.org/cellosaurus/CVCL_B0T9")
