		...
		if productNameArea!=None:
			...
			if sizeDataEl != None:
				...
				for index,size in enumerate(sizes):
					sizeAttr = size["attributes"]
					if "attribute_size" in sizeAttr:
						pInfo["size"+str(index)] = sizeAttr["attribute_size"]
					else:
						pInfo["size"+str(index)] = sizeAttr["attribute_pa_size"]
...
headers=[
	...
	,'Description','Uniprot link','Brand','Product type','Host Species','Product name','Expression system','Molecular weight'
	,'Purity estimated','Buffer','Form','Delivery condition','Storage condition','Reactivity','Applications','Fragment Type','Aliases /Synonyms'
	,'Clonality','Purification','Immunogen','Gene Name','Protein Name','Dilution','Concentration','Specificity',
	'Target species','Related Products','Publications','PublicationsLink','Note','datasheet','extradocument'
]

excelFname="proteogenix4.xlsx"
products = []
