...
def getArrayIndex(array, key, val):
...
def getTypeLeve1(url, typeCount, pInfo, products,totalLink):
	if len(url)>0:
		...
		if url not in totalLink:
			...
			temp= pInfo.copy()
			pIndex = getArrayIndex(products, 'link', url)
			temp["type"+str(typeCount)] = temp["type"+str(typeCount)]+ "【转-"+str(pIndex+2)+"行】"
			products[pIndex]["linkcount"] = int(products[pIndex]["linkcount"])+1
			products.append(temp.copy())
		...
		for type2 in typeChAreas:
			...
			if type3Area!=None:
				...
				if type2Link.name=="a":
					...
					if t2_pInfo["link"] not in totalLink:
				...
				totalLink.append(t2_pInfo["link"])
...
products = []
totalLink = []
# getTypeLeve1("https://www.thermofisher.com/us/en/home/clinical/cell-gene-therapy.html", 3, {"linkcount":0}, products,totalLink)
getProductType('https://www.thermofisher.com/us/en/home.html', products, totalLink)

headers=['linkcount','link','type1','type2','type3','type4','type5','type6','type7','type8','type9','type10','type11','type12','title','description']
